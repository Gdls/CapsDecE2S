...
class BertConfig(object):
  ...
  def __init__(self,
    ...
    self.lmms_emb = np.load("data/lmms/lmms1024_emb.npy")
  @classmethod
    ...
    with tf.variable_scope(scope, default_name="bert"):
      ...
      with tf.variable_scope("semantic_decomposing"):
        ...
        with tf.variable_scope("capsule_routing"):
            ...
            #b_IJ_q = tf.constant(0.0, dtype = 'float32', shape=[batch_size, MP_dim, MP_dim, 1, 1])

            b_IJ_p = tf.constant(np.zeros(shape = [1, MP_dim, MP_dim, 1, 1], dtype='float32'))
            capsules_q = match_utils.routing(q_sense, b_IJ_q, num_outputs=MP_dim, num_dims=shared_dim, iter_routing=3)
            capsules_q = tf.squeeze(tf.squeeze(capsules_q, axis=1), axis = -1) #batch_size, Mp, dim, 1 => batch_size, Mp, dim
            tf.get_variable_scope().reuse_variables()
            capsules_p = match_utils.routing(p_sense, b_IJ_p, num_outputs=MP_dim, num_dims=shared_dim, iter_routing=3)
            capsules_p = tf.squeeze(tf.squeeze(capsules_p, axis=1), axis = -1) #batch_size, Mp, dim, 1 => batch_size, Mp, dim
        '''
        ...
        q_m_sense = 0.3*capsules_q + 0.1*q_global_s + 0.6*q_local_s #batch, p, dim
        p_m_sense = 0.3*capsules_p + 0.1*p_global_s + 0.6*p_local_s

        #Only global context
        #q_m_sense = 0.6*capsules_q + 0.4*q_global_s #batch, p, dim
        #p_m_sense = 0.6*capsules_p + 0.4*p_global_s

        #Only local context
        #q_m_sense = 0.4*capsules_q + 0.6*q_local_s #batch, p, dim
        #p_m_sense = 0.4*capsules_p + 0.6*p_local_s
        ...
        #self.q_m_norm_weight = q_m_norm_weight
        #self.p_m_norm_weight = p_m_norm_weight
      ...
      #print("Capsule output:", self.pooled_output)
      #print ("Bert sqeeze embedding:",self.embedding_output_ids_a, self.embedding_output_ids_b)
      #print("Bert context:", self.sequence_output)
      #print("Context words in bert:", q_embedd_index,p_embedd_index)
      #pdb.set_trace()
  def get_pooled_output(self):
    return self.pooled_output
  def get_sememe_w(self):
    return [self.q_m_norm_weight, self.p_m_norm_weight]
